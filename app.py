...
def water_colour_effect(decode_array_to_img):
    rgb_effect = cv2.cvtColor(decode_array_to_img, cv2.COLOR_RGB2BGR)
...
def HDR(decode_array_to_img):
    hdr_effect = cv2.deailEnhance(decode_array_to_img, sigma_s = 12, sigma_r = 0.15)
